...
const PRODUCT_SLUG = 'divya-hawan-powder';
...
const FAQ_ITEMS = [
  {
    q: 'What is Divya Hawan Powder made of?',
    a: 'A traditional blend of herbs, wood, ghee-soaked samagri and havan ingredients, prepared following Vedic tradition — the same purity standard used across every Doshhmukti product.',
  },
  {
    q: 'How do I use it for a havan at home?',
    a: 'Light a small charcoal or dhoop base in a havan kund or steel plate, add a pinch of the powder at a time while chanting your mantra, and let the smoke circulate through the room. A few minutes daily or weekly is enough for regular home purification.',
  },
  {
...
const BENEFIT_ICONS = [Flame, Wind, Sparkles, ShieldCheck, FlameKindling, HeartHandshake];
...
// "You May Also Need" rail near the bottom — same protection/clarity purpose this
...
async function getRelatedProducts(currentSlug: string): Promise<Product[]> {
  try {
    const params = new URLSearchParams({ purpose: 'clarity', sort: 'newest', limit: '9' });
...
async function RelatedRail() {
  ...
  return <ProductRail eyebrow="More" title="Purification & Clarity" products={relatedProducts} tinted tightTop tightBottom />;
}
...
export default async function DivyaHawanPowderPage() {
  ...
  // strings, not numbers — Number(...) here is required, or `"350" > "299"` below does a
    ...
    <div className={`${fraunces.variable} bg-[#FFFDF8] text-[#2B1B0C] overflow-x-hidden`}>
      ...
      <section className="relative bg-[#2B1B0C] overflow-hidden pt-24 sm:pt-28">
        ...
        <span className="relative z-10 block text-center font-body text-[11px] sm:text-xs font-bold uppercase tracking-[0.35em] text-[#E6B873] mb-8 sm:mb-10">
          Vedic Fire Purification Ritual
        </span>
          ...
          <div className="relative z-10 aspect-square rounded-[2rem] sm:rounded-[2.5rem] overflow-hidden shadow-[0_40px_90px_-25px_rgba(0,0,0,0.55)] sm:shadow-[0_60px_120px_-30px_rgba(0,0,0,0.55)] animate-float motion-reduce:animate-none">
            ...
            <div className="hidden sm:block absolute left-3 bottom-3 z-20 w-[4.5rem] h-[4.5rem]">
              <svg viewBox="0 0 100 100" className="w-full h-full animate-[spin_18s_linear_infinite] motion-reduce:animate-none">
                <defs>
                  <path id="dhp-badge-circle" d="M 50,50 m -38,0 a 38,38 0 1,1 76,0 a 38,38 0 1,1 -76,0" />
                </defs>
                ...
                <text fill="#E6B873" fontSize="8.2" fontWeight="700" letterSpacing="1.5">
                  <textPath href="#dhp-badge-circle" startOffset="0%">
                    PURE SAMAGRI • VEDIC •&nbsp;
                  </textPath>
                </text>
              </svg>
              <span className="absolute inset-0 flex items-center justify-center">
                <Flame className="w-5 h-5 text-[#E6B873]" strokeWidth={1.5} />
              </span>
            </div>
      ...
      <section className="relative px-5 pt-6 sm:pt-8 pb-24 sm:pb-32 overflow-hidden">
        ...
        <Reveal className="max-w-2xl mx-auto text-center mb-14 sm:mb-16">
          ...
          <h2
            ...
            style={{ fontFamily: 'var(--font-fraunces), Georgia, serif' }}
          >
            A cleaner space, <span className="italic font-light text-[#9C5A26]">every time you light it.</span>
          </h2>
        </Reveal>

        <StaggerGroup className="relative max-w-5xl mx-auto grid sm:grid-cols-2 lg:grid-cols-3 gap-5 sm:gap-6 items-stretch">
          {benefits.map((b, i) => {
            const Icon = BENEFIT_ICONS[i % BENEFIT_ICONS.length] ?? Flame;
            return (
      ...
      {/* ─── Acharya Madhav — havan/purification-angled variant of the site-wide AI astrologer ── */}
      <AcharyaSection />
